...
def dump_irr_412_file(outfile, p, Pres, Value, Qc, metatata, mode='w'):
    ...
    print "dumping irr_412 on " + outfile
    irr_412_already_existing="nIRR_412" in ncOUT.dimensions.keys()
    if not irr_412_already_existing : ncOUT.createDimension('nIRR_412', nP)
    ncvar=ncOUT.createVariable("PRES_IRR_412", 'f', ('nIRR_412',))
    ncvar[:]=Pres
    ncvar=ncOUT.createVariable("IRR_412", 'f', ('nIRR_412',))
    ncvar[:]=Value
    if not irr_412_already_existing:
        setattr(ncvar, 'origin'     , metadata.origin)
        setattr(ncvar, 'file_origin', metadata.filename)
        setattr(ncvar, 'variable'   , 'IRR_412')
        setattr(ncvar, 'units'      , "W/m^2/nm")
    ncvar=ncOUT.createVariable("IRR_412_QC", 'f', ('nIRR_412',))
    ncvar[:]=Qc
    ncOUT.close()
...
force_writing_irr_412=args.force

PROFILES_OPT_2019 =optbio_float_2019.FloatSelector('IRR_412', TI, R)
...
for iwmo, wmo in enumerate(wmo_list):
    ...
    for ip, p in enumerate(Profilelist):
        ...
        if superfloat_generator.exist_valid(outfile):
            if not superfloat_generator.exist_variable('IRR_412', outfile):
                Pres, Value, Qc = p.read('IRR_412')
                if Pres is not None: dump_irr_412_file(outfile, p, Pres, Value, Qc, metadata,mode='a')
            else:
                if force_writing_irr_412:
                    Pres, Value, Qc = p.read('IRR_412')
                    if Pres is not None: dump_irr_412_file(outfile, p, Pres, Value, Qc, metadata,mode='a')
        else:
            print outfile + "not existing"
             #Pres, Value, Qc = p.read('RR_412')
             #if Pres is not None: dump_irr_412_file(outfile, p, Pres, Value, Qc, metadata,mode='w')
