...
def dump_par_file(outfile, p, Pres, Value, Qc, metatata, mode='w'):
    ...
    if not par_already_existing : ncOUT.createDimension('nPAR', nP)
    ncvar=ncOUT.createVariable("PRES_PAR", 'f', ('nPAR',))
    ncvar[:]=Pres
    ncvar=ncOUT.createVariable("PAR", 'f', ('nPAR',))
    ncvar[:]=Value
    if not par_already_existing:
        setattr(ncvar, 'origin'     , metadata.origin)
        setattr(ncvar, 'file_origin', metadata.filename)
        setattr(ncvar, 'variable'   , 'PAR')
        setattr(ncvar, 'units'      , "milligram/m3")
    ncvar=ncOUT.createVariable("PAR_QC", 'f', ('nPAR',))
    ncvar[:]=Qc
    ncOUT.close()
...
PROFILES_OPT_2019 =optbio_float_2019.FloatSelector('PAR', TI, R)
...
for iwmo, wmo in enumerate(wmo_list):
    ...
    for ip, p in enumerate(Profilelist):
        ...
        if superfloat_generator.exist_valid(outfile):
            if not superfloat_generator.exist_variable('PAR', outfile):
                Pres, Value, Qc = p.read('PAR')
                if Pres is not None: dump_par_file(outfile, p, Pres, Value, Qc, metadata,mode='a')
            else:
