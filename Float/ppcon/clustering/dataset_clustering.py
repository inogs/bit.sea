...
class FloatDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            self.name = list(dict(self.df).keys())[index + 1][-13:]
        ...
        nitrate = from_string_to_tensor(self.samples[7])
        chla = from_string_to_tensor(self.samples[8])
        BBP700 = from_string_to_tensor(self.samples[9])

        # label = label_doxy * label_psal * label_temp  # the label is equal to one only if I have data valid for all
        # depth

        return year, day_rad, lat, lon, temp, psal, doxy, nitrate, chla, BBP700, self.name
