...
class figure_generator():
    ...
    def _gen_structure(self, IDrun,season,subbasin_name,var_list, TEXTxlabel,xmin,xmax,xinc=None):
        ...
        * subbasin_name * string
        ...
        '''
        fig, axs = pl.subplots(2,4, facecolor='w', edgecolor='k')
        axs = axs.ravel()
        ...
        axs[4].imshow(smaskplot, extent=[lon_min, lon_max, lat_max, lat_min])
        axs[4].invert_yaxis()
        axs[4].set_xlim([-6, 36])
        axs[4].set_ylim([30, 46])
        c_lon, c_lat=coastline.get()

        axs[4].plot(c_lon,c_lat, color='#000000',linewidth=0.5) # can generate Warning because of nans
        axs[4].xaxis.set_tick_params(labelsize=6)
        axs[4].yaxis.set_tick_params(labelsize=6)
        axs[4].set_xticks([ 0,10,20,30])
        axs[4].set_yticks([32,36,40,44])

        BBox = axs[4].get_position() # shift only the map a bit to left
        rect = [BBox.xmin+x_shift, BBox.ymin, BBox.width, BBox.height]
        axs[4].set_position(rect)
...
#        fig.text(0.12,0.15,IDrun,fontsize=15)
        ...
        axs[5].set_yticks([-2000,-1500,-1000,-500])
        axs[5].yaxis.set_tick_params(labelsize=8)
