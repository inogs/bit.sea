...
for i in range(len(varlist)):
    ...
    ax.scatter(MONTHS-0.25, FLOAT_W_mean, s=15, color='darkblue'    ,     label='float W')
    ax.scatter(MONTHS-0.15, MODEL_W_mean, s=15, color='dodgerblue'  ,     label='model W') 

    ax.scatter(MONTHS+0.15, FLOAT_E_mean, s=15, color='purple'     ,     label='float E')
    ax.scatter(MONTHS+0.25, MODEL_E_mean, s=15, color='palevioletred'  ,     label='model E')   

    ax.errorbar(MONTHS-0.25, FLOAT_W_mean, yerr=FLOAT_W_std, color='darkblue'   , fmt='o')
    ax.errorbar(MONTHS-0.15, MODEL_W_mean, yerr=MODEL_W_std, color='dodgerblue' , fmt='o')

    ax.errorbar(MONTHS+0.15, FLOAT_E_mean, yerr=FLOAT_E_std, color='purple'    , fmt='o')
    ax.errorbar(MONTHS+0.25, MODEL_E_mean, yerr=MODEL_E_std, color='palevioletred' , fmt='o')
    ...
    plot_out = 'PLOTS/' + SIM_NAME.replace('/', '_') + arglist[i] + '_scatter_MONTHLY_WE.png'
