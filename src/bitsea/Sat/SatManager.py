...
def readfromfile(filename, var="CHL"):
    ...
    if ndims == 2:
        CHL_IN = np.array(varObj)
    if ndims == 3:
        CHL_IN = np.array(varObj[0, :, :])
    del varObj
    ncIN.close()
    return CHL_IN
...
def readBathymetry(
    filename="/gss/gss_work/DRES_OGS_BiGe/Observations/TIME_RAW_DATA/STATIC/SAT/MODIS/Bathy_MODIS.nc",
):
    ncIN = NC.Dataset(filename, "r")
...
def interpOnV1(CHL_16, bathy_threshold=0):
    ...
    # this is just a hint of how to develop/simplify the next part

    PREVLON = (
        np.array(
            [getnextIndex(NativeMesh.lon, V1.lon[i]) for i in range(V1.jpi)],
            np.int32,
        )
        - 1
    )
    PREVLAT = (
        np.array(
            [getnextIndex(NativeMesh.lat, V1.lat[i]) for i in range(V1.jpj)],
            np.int32,
        )
        - 1
    )
...
def WeightedLogAverager(M, w):
    ...
    nFrames = M.shape[0]
    maskM = np.logical_not(M == fillValue)
...
def writing_mode(filename):
    ...
    """
    writingmode = "w"
    if exist_valid(filename):
        writingmode = "a"
    return writingmode


def write_coords_in_sat_file(file_pointer, mesh=OneKmMesh):
    dims = {"time": 1, "depth": 1, "lon": mesh.jpi, "lat": mesh.jpj}
    for dim_name, dim_value in dims.items():
        file_pointer.createDimension(dim_name, dim_value)
    ...
    for coord_name, coord_value in coords_values.items():
        ncvar = file_pointer.createVariable(coord_name, "f", (coord_name,))
        ncvar[:] = coord_value
...
def dumpGenericfile(
    ...
    * mode     * string, 'w' or 'a'
    ...
    with NC.Dataset(filename, mode) as ncOUT:
        if mode == "w":
            write_coords_in_sat_file(ncOUT, mesh)
