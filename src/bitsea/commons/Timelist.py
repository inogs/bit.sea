...
def computeTimeWindow(freqString, currentDate):
    if freqString == "daily":
        req = requestors.Daily_req(
            currentDate.year, currentDate.month, currentDate.day
        )
    if freqString == "weekly":
        req = requestors.Weekly_req(
            currentDate.year, currentDate.month, currentDate.day
        )
    if freqString == "monthly":
        req = requestors.Monthly_req(currentDate.year, currentDate.month)
    if freqString == "yearly":
        req = requestors.Yearly_req(currentDate.year)
    if freqString.startswith("days"):
        pos = freqString.find("=")
        ndays = int(freqString[pos + 1 :])
        req = requestors.Interval_req(
            currentDate.year, currentDate.month, currentDate.day, days=ndays
        )
    if freqString.startswith("hours"):
        pos = freqString.find("=")
        nhours = int(freqString[pos + 1 :])
        req = requestors.Hourly_req(
            currentDate.year,
            currentDate.month,
            currentDate.day,
            currentDate.hour,
    ...
    if freqString.startswith("seconds="):
        ...
        req = requestors.seconds_req(
            currentDate.year,
            currentDate.month,
            currentDate.day,
            currentDate.hour,
            currentDate.minute,
            delta_seconds=nseconds,
        )
    return TimeInterval.fromdatetimes(
        req.time_interval.start_time, req.time_interval.end_time
    )
...
class TimeList:
    def __init__(self, datelist, forceFrequency=None):
        ...
        if forceFrequency is not None:
            ...
            if self.nTimes > 1:
                ...
                self._timeinterval = TimeInterval.fromdatetimes(
    ...
    def timeinterval(self):
        if self._timeinterval is None:
            raise RuntimeError(
                "timeinterval has not been initialized; this list has less "
                "than 2 elements or the frequency has been forced manually"
            )
    ...
    def fromfilenames(
        ...
        * forceFrequency * string, like 'daily','weekly','monthly'. The default is None.
        ...
        for pathfile in filelist_ALL:
            ...
            try:
                actualtime = datetime.datetime.strptime(
                    datestr, IOname.dateformat
                )
        ...
        if forcehour is not None:
            for iFrame, t in enumerate(TimeListObj.Timelist):
                newt = datetime.datetime(
                    t.year, t.month, t.day, forcehour, 0, 0
                )
    ...
    def __searchFrequency(self):
        ...
        if len(self.Timelist) < 2:
            timestr = self.timeinterval.start_time.strftime(
                " between %Y%m%d and "
            ) + self.timeinterval.end_time.strftime("%Y%m%d")
            print(
                "Frequency cannot be calculated in " + self.inputdir + timestr
            )
            return None
        ...
        if (days > 6) & (days < 8):
            return (
                "weekly"  # centered in " + str(self.Timelist[1].isoweekday())
            )
        if (days > 26) & (days < 32):
            return "monthly"
        if (days < 1) & (days >= 1.0 / 24.0):
            return "hours=%d" % int(days * 24)
        if days < 1.0 / 24:
            return "seconds=%d" % int(days * 24.0 * 3600.0)
    ...
    def __generaltimeselector(self, requestor):
        ...
        if self.inputFrequency in [
            ...
            for it, t in enumerate(self.Timelist):
                ...
                t2 = TimeInterval.fromdatetimes(
                    requestor.time_interval.start_time,
                    requestor.time_interval.end_time,
                )
            ...
            if self.inputFrequency in ["daily", "days=2"]:
                for it, t in enumerate(self.Timelist):
                    if requestor.time_interval.contains(t):
                        SELECTION.append(it)
                        dayd = abs(
                            (t - requestor.time_interval.start_time).days - 3
                        )
    ...
    def select(self, requestor):
        ...
        if isinstance(requestor, requestors.Daily_req):
            ...
            assert (
                self.inputFrequency in ["daily", "seconds=900", "seconds=1800"]
            ) | (self.inputFrequency.startswith("hours"))
            SELECTION = []
            weights = []
            for it, t in enumerate(self.Timelist):
        ...
        if isinstance(requestor, requestors.Monthly_req):
            ...
            if (
                ...
                for it, t in enumerate(self.Timelist):
                    if (t.year == requestor.year) & (
                        t.month == requestor.month
                    ):
                        SELECTION.append(it)
                        weights.append(1.0)
                return SELECTION, np.array(weights)
        ...
        if isinstance(requestor, requestors.Weekly_req):
            ...
            condition = (
                (self.inputFrequency in ["daily", "days=2"])
                | (self.inputFrequency.startswith("hours"))
        ...
        if isinstance(requestor, requestors.Clim_season):
            ...
            YEARLIST = self.getYearlist()
            for year_req in YEARLIST:
                req = requestors.Season_req(
                    year_req.year, requestor.season, requestor.seasonobj
                )
                s, w = self.select(req)
                SELECTION.extend(s)
                weights.extend(w)
    ...
    def getWeeklyList(self, weekday):
        ...
        for counter, day in enumerate(range(weekday - 3, weekday + 4)):
            ...
            if interested_weekday == 0:
                interested_weekday = 7
            if interested_weekday == self.timeinterval.start_time.isoweekday():
                index = counter

        starting_centered_day = (
            self.timeinterval.start_time
            + datetime.timedelta(days=PossibleShifts[index])
        )
    ...
    def getMonthlist(self, extrap=False):
        ...
        for t in self.Timelist:
            ...
            if newmonth not in MONTH_LIST:
                MONTH_LIST.append(newmonth)
        if extrap:
            pass
        else:
            MONTH_LIST_RED = []
            firstMonth = datetime.datetime(
                self.timeinterval.start_time.year,
                self.timeinterval.start_time.month,
                1,
            )
            lastMonth = datetime.datetime(
                self.timeinterval.end_time.year,
                self.timeinterval.end_time.month,
                1,
            )
    ...
    def getYearlist(self):
        ...
        for t in self.Timelist:
            if t.year not in YEARLIST:
                YEARLIST.append(t.year)
        REQ_LIST = []
    ...
    def getSeasonList(self, seasonobj=seasonobj, extrap=False):
        ...
        for t in self.Timelist:
            ...
            if newSeason not in SEASON_LIST:
                SEASON_LIST.append(newSeason)
        if extrap:
            pass
        else:
            SEASON_LIST_RED = []
            firstSeason = requestors.Season_req(
                self.timeinterval.start_time.year,
                seasonobj.findseason(self.timeinterval.start_time),
                seasonobj,
            )
            lastSeason = requestors.Season_req(
                self.timeinterval.end_time.year,
                seasonobj.findseason(self.timeinterval.end_time),
                seasonobj,
            )
            for season in SEASON_LIST:
                ...
                if (
                    req.time_interval.start_time
                    >= firstSeason.time_interval.start_time
                ) & (
                    req.time_interval.end_time
                    <= lastSeason.time_interval.end_time
                ):
    ...
    def getOwnList(self):
        ...
        if self.inputFrequency.startswith("seconds="):
            ...
            for t in self.Timelist:
                REQ_LIST.append(
                    requestors.seconds_req(
                        t.year, t.month, t.day, t.hour, t.minute, nSeconds
                    )
                )
            return REQ_LIST
        if self.inputFrequency.startswith("hours"):
            ...
            for t in self.Timelist:
                REQ_LIST.append(
                    requestors.Hourly_req(
                        t.year, t.month, t.day, t.hour, delta_hours=nHours
                    )
                )
            return REQ_LIST
        if self.inputFrequency == "daily":
        ...
        if self.inputFrequency == "10days":
            for t in self.Timelist:
                REQ_LIST.append(
                    requestors.Interval_req(t.year, t.month, t.day, "days=10")
                )
            return REQ_LIST
        if self.inputFrequency == "monthly":
            for t in self.Timelist:
    ...
    def getSpecificIntervalList(
        self, deltadays=10, starttime="19971001-12:00:00"
    ):
        """
        ...
        dl = DL.getTimeList(
            starttime,
            self.timeinterval.end_time.strftime("%Y%m%d-%H:%M:%S"),
            deltastr,
        )
        for dateobj in dl:
            req = requestors.Interval_req(
                dateobj.year, dateobj.month, dateobj.day, deltadays
            )
            REQ_LIST.append(req)
        return REQ_LIST
        ...
        * Coupled_List * a list of tuples (datetime object, list_of_indices)
        ...
        for ir, req in enumerate(self.getOwnList()):
            ...
            if len(LIST_of_IND) > 0:
                Coupled_List.append((self.Timelist[ir], LIST_of_IND))
        return Coupled_List
    ...
    def get_datetime_array(self, resolution="s"):
        ...
        """
        output = np.empty(
            (len(self.Timelist),), dtype=f"datetime64[{resolution}]"
        )

        for i, t in enumerate(self.Timelist):
            numpy_t = np.datetime64(t.strftime("%Y-%m-%dT%H:%M:%S"), resolution)
            output[i] = numpy_t
