    ...
    def __init__(
        ...
        if filename is None:
            ...
            if one_dimensions_ind.size == 0:  # no dimensions to remove
                self.__values = np.asarray(dset.variables[v])
            if one_dimensions_ind.size == 1:
                if one_dimensions_ind[0] == 0:
                    self.__values = np.asarray(dset.variables[v][0, :])
            ...
            self.__values = np.asarray(dset.variables[v][tuple(v_slices)])
        ...
        if self.dims == 3:
            ...
            if data_jpk > mask_jpk:  # working with reduced mask
                if verbose:
                    warnings.warn(
                        "WARNING: slicing 3D field in range 0 - {}".format(
                            mask_jpk
                        )
                    )
