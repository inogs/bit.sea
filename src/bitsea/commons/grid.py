...
class SidesCalculator(ABC):
    """
    A `SideCalculator` computes the size of the grid cell sides based
    on the coordinates of their centers. It provides the size of the
    longitudinal side (east-west) and the latitudinal side
    (north-south) for each cell in the grid.
    ...
    can vary across the cell).
    To accommodate this, various implementations are provided, each
    making different assumptions and employing different algorithms.

    All the distances returned by a SidesCalculator are in meters.
    """
    @abstractmethod
    def __call__(self, xlevels: np.ndarray, ylevels: np.ndarray)\
...
class NemoGridSidesCalculator(SidesCalculator):
    ...
    model to compute the size of grid cells along longitude and latitude
    when the grid is regular.
...
class GeoPySidesCalculator:
    """
    This algorithm estimates the lengths of the cell sides by
    calculating the positions of the cell faces as the midpoints
    between centers (using the `_extend_from_average` function).
    It then computes the distance between the two opposite faces of
    each cell.
...
class Grid(ABC):
    ...
    def e1t(self) -> np.ndarray:
        """
        A 2D Numpy array such that `e1t[i, j]` is the length of the
        segment of the parallel passing through the cell centered at
        `(xlevels[i, j], ylevels[i, j])` that lies within the cell
        (in meters).
        """
        raise NotImplementedError
    ...
    def e2t(self):
        """
        A 2D Numpy array such that `e2t[i, j]` is the length of the
        segment of the meridian passing through the cell centered at
        `(xlevels[i, j], ylevels[i, j])` that lies within the cell
        (in meters).
        """
        raise NotImplementedError
    ...
    def area(self):
        """
        A 2D numpy array such that area[i, j] is the area of the cell
        `(i, j)` (in square meters).
        """
    ...
    def _build_e_t_arrays(self, algorithm: GridSidesAlgorithm) -> Tuple[np.ndarray, np.ndarray]:
...
class IrregularGrid(Grid):
    ...
    def _build_and_store_e_t_arrays(self):
        e1t, e2t = self._build_e_t_arrays(
            algorithm=GridSidesAlgorithm.GEODESIC
        )
...
class RegularGrid(Grid, Regular):
    ...
    Args:
        ...
        e2t (2D array): is like e1t but along the meridian. It also can
          be `None`.
    """
    def __init__(self, *, lon, lat, e1t: Optional[np.ndarray] = None,
                 e2t: Optional[np.ndarray] = None):
    ...
    def _build_and_store_e_t_arrays(self):
        e1t, e2t = self._build_e_t_arrays(
            algorithm=GridSidesAlgorithm.NEMO
        )
        if self._e1t is None:
            self._e1t = e1t
            self._e1t.setflags(write=False)
