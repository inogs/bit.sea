...
def mapplot(
    ...
    if mask is not None:
        ...
        if not ((coastline_lon is None) or (coastline_lat is None)):
            ...
            # Draw coastline
            ax.plot(
                coastline_lon, coastline_lat, color="#000000", linewidth=0.5
            )
            ax.set_xlim([-6, 36])
            ax.set_ylim([30, 46])
...
def mapplotlog(
    ...
    # Set color bar
    im.set_clim(clim[0], clim[1])
    cbar_ticks_list = np.logspace(
        np.log10(clim[0]), np.log10(clim[1]), cbar_ticks
    ).tolist()
    cbar_ticks_labels = list()
    for t in cbar_ticks_list:
...
def mapplot_medeaf(
    map_dict, fig, ax, mask=None, ncolors=256, background_img=None
):
    """
    ...
    font_prop = font_manager.FontProperties(
        fname="TitilliumWeb-Regular.ttf", size=14
    )
    font_prop13 = font_manager.FontProperties(
        fname="TitilliumWeb-Regular.ttf", size=13
    )
    ...
    im = ax.imshow(
        map_dict["data"], extent=[lon_min, lon_max, lat_max, lat_min], cmap=cmap
    )
    ...
    cax = fig.add_axes((0.88, 0.13, 0.03, 0.78))
    fig.colorbar(im, cax=cax, ticks=cbar_ticks_list)
...
def mapplot_medeaf_V5C(
    map_dict, map_obj, maskobj, fig, ax, ncolors=256, logo=None
):
    """
    Designed for web site
    """
    ...
    font_prop = font_manager.FontProperties(
        fname="TitilliumWeb-Bold.ttf", size="xx-large", weight="bold"
    )
    font_prop13 = font_manager.FontProperties(
        fname="TitilliumWeb-Regular.ttf", size=13
    )
    ...
    if (fig is None) or (ax is None):
        fig, ax = pl.subplots()
        fig.set_size_inches(
            10, 10 * map_obj.ymax / map_obj.xmax * (0.85 / 0.85)
        )
    ...
    R10 = Rectangle(
        27.7687, 28.845, 40.0348, 40.2938
    )  # 3 laghi vicino bursa, turchia
    ...
    map2d = map_dict["data"]
    map2d[map2d > 1.0e19] = np.nan
    Zm = np.ma.masked_invalid(map2d)
    cs = map_obj.pcolormesh(
        maskobj.xlevels,
        maskobj.ylevels,
        Zm,
        cmap=cmap,
    ...
    parallels = np.arange(32.0, 46.0, 4)
    h_dict = map_obj.drawparallels(
        parallels,
        labels=[1, 0, 0, 1],
        linewidth=0.5,
        fontsize=13,
    ...
    h_dict = map_obj.drawparallels(
        parallels, labels=[1, 0, 0, 1], fontsize=13, dashes=[6, 900], ax=ax
    )
    set_invisible(h_dict)
    # draw meridians
    meridians = np.arange(-4.0, 40, 4.0)
    set_font(
        map_obj.drawmeridians(
            meridians,
            labels=[0, 0, 0, 1],
            linewidth=0.5,
            fontsize=13,
    ...
    set_invisible(
        map_obj.drawmeridians(
            meridians, labels=[0, 0, 0, 1], fontsize=13, dashes=[6, 900], ax=ax
        )
    )
    ...
    cbar = map_obj.colorbar(
        cs, location="right", size="5%", pad="2%", ticks=cbar_ticks_list
    )
    ...
    t = ax.set_xlabel(
        "Longitude", size="xx-large", fontweight="bold", labelpad=20
    )  # ).set_fontsize(15)
    t.set_font_properties(font_prop)
    t = ax.set_ylabel(
        "Latitude", size="xx-large", fontweight="bold", labelpad=40
    )  # ).set_fontsize(15)
    t.set_font_properties(font_prop)
...
def generic_mapplot_medeaf(
    map_dict, map_obj, maskobj, fig=None, ax=None, ncolors=256, logo=None
):
    """
    Designed for web site
    """
...
def mapplot_onlycolor(
    map_dict, fig, ax, mask=None, ncolors=256, cbar_ticks=5, dpi=72.0
):
...
if __name__ == "__main__":
    ...
    map_dict = {
        "data": map2d,
        "clim": [0, 0.1],
        "date": datetime.strptime("20160116", "%Y%m%d"),
        "varname": "N1p",
        "layer": Layer(0, 10),
        "longname": "Phosphate",
        "units": "mmolP/m3",
