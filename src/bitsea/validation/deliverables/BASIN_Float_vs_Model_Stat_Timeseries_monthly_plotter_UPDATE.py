...
BIAS_matrix = np.load(INDIR + 'Basin_Statistics_BIAS.npy')
RMSD_matrix = np.load(INDIR + 'Basin_Statistics_RMSD.npy')
CORR_matrix = np.load(INDIR + 'Basin_Statistics_CORR.npy')
...
#            TABLE_METRICS[iSub,6] = m.RMSE()
            ...
            TABLE_METRICS[iSub,5] = np.nanmean(BIAS_matrix[ivar,:,iSub,0])
            TABLE_METRICS[iSub,6] = np.nanmean(RMSD_matrix[ivar,:,iSub,0])
...
#            TABLE_METRICS[iSub,18] = m.RMSE()
            ...
            TABLE_METRICS[iSub,7] = np.nanmean(A_float[ivar,3:10,iSub,2]) # DCM Float mean
            TABLE_METRICS[iSub,8] = np.nanmean(A_model[ivar,3:10,iSub,2]) # DCM Model mean
            TABLE_METRICS[iSub,9] = np.nanstd(A_float[ivar,3:10,iSub,2])  # DCM Float std
            TABLE_METRICS[iSub,10] = np.nanstd(A_model[ivar,3:10,iSub,2]) # DCM Model std

            TABLE_METRICS[iSub,11] = np.nanmean(BIAS_matrix[ivar,3:10,iSub,2]) # DCM bias
            TABLE_METRICS[iSub,12] = np.nanmean(RMSD_matrix[ivar,3:10,iSub,2]) # DCM rmse

            TABLE_METRICS[iSub,13] = np.nanmean(A_float[ivar,0:3,iSub,3]) # WBL Float mean
            TABLE_METRICS[iSub,14] = np.nanmean(A_model[ivar,0:3,iSub,3]) # WBL Model mean
            TABLE_METRICS[iSub,15] = np.nanstd(A_float[ivar,0:3,iSub,3]) # WBL Float std
            TABLE_METRICS[iSub,16] = np.nanstd(A_model[ivar,0:3,iSub,3]) # WBL Model std

            TABLE_METRICS[iSub,17] = np.nanmean(BIAS_matrix[ivar,0:3,iSub,3]) # WBL bias
            TABLE_METRICS[iSub,18] = np.nanmean(RMSD_matrix[ivar,0:3,iSub,3]) # WBL rmse
...
#            TABLE_METRICS[iSub,30] = m.RMSE()
            TABLE_METRICS[iSub,29] = np.nanmean(BIAS_matrix[ivar,:,iSub,7])
            TABLE_METRICS[iSub,30] = np.nanmean(RMSD_matrix[ivar,:,iSub,7])
...
#            TABLE_METRICS[iSub,36] = m.RMSE()
            ...
            TABLE_METRICS[iSub,35] = np.nanmean(BIAS_matrix[ivar,:,iSub,8])
            TABLE_METRICS[iSub,36] = np.nanmean(RMSD_matrix[ivar,:,iSub,8])
