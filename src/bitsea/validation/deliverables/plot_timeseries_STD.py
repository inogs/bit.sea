...
def lims_and_colors(var,zone,sub):
    vmin=0.0
    vmax=0.25
    color="tab:green"
    ...
    if (args.var == "P_l"): 
       ...
       if (args.zone == "rivers"):
           vmax=1.2
       if sub.name.startswith("Po"):
           vmax=4.0    
       if sub.name.startswith("Piave"):
           vmax=2.0
    if (args.var == "kd490"):
       vmin=0.02
       vmax=0.12
    ...
    if (args.var.startswith('RRS')):
        vmin=0.0
        vmax=0.025
        if args.var.startswith(('RRS5', 'RRS6')):
           vmax=0.01
        color="tab:blue"
        lightcolor="lightsteelblue"

    return(vmin,vmax,color,lightcolor) 
...
for isub,sub in enumerate(OGS.P):
    ...
    vmin,vmax,color,lightcolor = lims_and_colors(args.var,args.zone,sub)
