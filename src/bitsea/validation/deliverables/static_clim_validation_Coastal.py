...
LayerList = [Layer(0,30), Layer(30,60), Layer(60,100), Layer(100,150), Layer(150,200)]#Layer(150,300), Layer(300,600), Layer(600,1000)]
...
#    else: SUBlist = basV2.Pred.basin_list
    CLIM_REF_static,_ = climatology.get_climatology_coast(var,SUBlist, LayerList,TheMask, basin_expand=False,QC=False)
    nSub = len(SUBlist)
    CLIM_MODEL = np.zeros((nSub, nLayers))
    for iSub, sub in enumerate(SUBlist):
        print (sub.name)
        Mean_profiles,_,_ = Hovmoeller_matrix(IMESERIES,TL, np.arange(jpk), iSub, icoast=0, istat=0)
...
#    if ( var in ["N1p","N3n","N5s","O2o","O3c","O3h"] ):
    CLIM_REF_static,_ = climatology.get_climatology_coast(var,SUBlist, LayerList_2, TheMask,basin_expand=False,QC=False)
#    else:
#        CLIM_REF_static,_ = climatology.get_climatology(var,SUBlist, LayerList_2)

    CLIM_MODEL = np.zeros((nSub, nLayers))
    for iSub, sub in enumerate(SUBlist):
        Mean_profiles,_,_ = Hovmoeller_matrix(TIMESERIES,TL, np.arange(jpk), iSub, icoast=0, istat=0)
        mean_profile = Mean_profiles.mean(axis=1)
        mean_profile[mean_profile==0]=np.nan
