...
# for wmo in ['6901510']:
    ...
    # for irun,run in enumerate(['CR']):
        ...
        times[run] = [p.time for p in wmo_track_list]
        ...
        # for irun,run in enumerate(['CR']):
            ...
            if (~np.isnan(model).all() == True) or (~np.isnan(ref).all() == True):
                ...
                if irun==0:
                    ...
                    axes[0].plot(times[run],ref,'o-g',label='Float')
                axes[0].plot(times[run],model,'-',color=colorList[run],label=run)
                axes[0].set_title('Integral 0-150m',fontsize=10)

                if irun==0:
                    print('irun---------------')
                    print(irun)
                    axes[1].plot(times[run],surf_ref,'o-g',label='Float')
                axes[1].plot(times[run],surf_model,'-',color=colorList[run],label=run)
