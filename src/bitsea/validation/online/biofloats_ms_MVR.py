...
for ivar, var in enumerate(VARLIST):
    ...
    for isub, sub in enumerate(OGS.MVR):
        ...
        Profilelist = bio_float.remove_bad_sensors(
            Profilelist_all, FLOATVARS[var]
        )
        ...
        for ilayer, layer in enumerate(LAYERLIST):
            ...
            REF_LAYER_MEAN = []
            CLIMA_LAYER_MEAN = []
            for floatmatchup, floatmatchup_clim in zip(
                Matchup_object_list, Matchup_object_list_clim
            ):
            ...
            if len(MODEL_LAYER_MEAN) > 2:
                ...
                if args.basedir_clim is not None:
                    M_LAYER_ANOMALY = matchup(
                        np.array(MODEL_LAYER_MEAN) - np.array(CLIMA_LAYER_MEAN),
