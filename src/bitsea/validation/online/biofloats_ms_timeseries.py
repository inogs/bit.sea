...
class timelistcontainer:
    def __init__(
        self, Ti, ARCHIVEDIR, searchstring, prefix="BioFloat_Weekly_validation_"
    ):
        self.timelist = []
        self.filelist = []
        self.bias = None
        ...
        TL = TimeList.fromfilenames(
            Ti, ARCHIVEDIR, searchstring, prefix=prefix, dateformat="%Y%m%d"
        )
    ...
    def initold(self, Ti, ARCHIVE_DIR, postfix_dir=""):
        ...
        TS = TimeSeries(
            ...
            glob_pattern="BioFloat_Weekly_validation",
        )
    ...
    def append_file(self, filename):
        ...
        self.number = np.concatenate(
            (
                self.number,
                number.reshape((1, self.nVAR, self.nSUB, self.nDEPTH)),
            ),
            axis=0,
        )
        self.bias = np.concatenate(
            (self.bias, bias.reshape((1, self.nVAR, self.nSUB, self.nDEPTH))),
            axis=0,
        )
        self.rmse = np.concatenate(
            (self.rmse, rmse.reshape((1, self.nVAR, self.nSUB, self.nDEPTH))),
            axis=0,
        )
        self.timelist.append(time)
