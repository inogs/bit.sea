...
def argument():
    ...
    parser.add_argument(
        "--submaskdir", "-b", type=str, default=None, required=True, help=""
    )
...
TLmis = TimeList.fromfilenames(
    None, INDIR, "*.npy", prefix="", dateformat="%Y%m%d"
)
TI = TimeInterval(
    TLmis.Timelist[0].strftime("%Y%m%d"), TLmis.Timelist[-1].strftime("%Y%m%d")
)

TLstats = TimeList.fromfilenames(
    TI,
    INCHECKDIR,
...
for sub in V2.P:
    ...
    for ii, masktype in enumerate(["All", "Open"]):
        plt.plot(
            TLmis.Timelist,
            LISTmis[sub.name][ii],
            color=DICTcol[masktype],
            label=masktype,
        )
        plt.plot(
            TLmis.Timelist[-1],
            LISTmis[sub.name][ii][-1],
            "o",
            markeredgecolor="red",
            markerfacecolor=DICTcol[masktype],
        )
    plt.legend(loc="upper left")
    plt.title(
        sub.name
        + r" Mean RMSD $[mgchl/m^3]$ - Last date "
    ...
    plt.bar(
        TLmis.Timelist,
        100.0 * np.array(LISTmis[sub.name][3]),
        width=3,
        label="% Weekly coverage "
        + "%1.f" % (100.0 * np.nanmean(LISTmis[sub.name][3])),
    )
    plt.legend(loc="upper left")
    plt.grid()
    ...
    plt.bar(
        TLstats.Timelist, LISTst[sub.name][1], width=1, label="Excluded Obs"
    )
    plt.legend(loc="upper left")
    plt.grid()
