...
RMSD_matrix = np.zeros((nVar, nTime, nSub, nStat), np.float32 ) * np.nan
BIAS_matrix = np.zeros((nVar, nTime, nSub, nStat), np.float32 ) * np.nan
CORR_matrix = np.zeros((nVar, nTime, nSub, nStat), np.float32 ) * np.nan
...
#		GM = M.getMatchups([p], TheMask.zlevels, var_mod, read_adjusted=adj, interpolation_on_Float=False)
            ...
            for iStat, sStat in enumerate(METRICS):
              #  if (iStat == 6): continue
              if (iStat == 6):
                A_float[ivar,itime,iSub,iStat] = np.sum(~np.isnan(Flo[:,0]))
                A_model[ivar,itime,iSub,iStat] = np.sum(~np.isnan(Flo[:,0]))
                BIAS_matrix[ivar,itime,iSub,iStat] = np.sum(~np.isnan(Flo[:,0]))
                RMSD_matrix[ivar,itime,iSub,iStat] = np.sum(~np.isnan(Flo[:,0]))
                CORR_matrix[ivar,itime,iSub,iStat] = np.sum(~np.isnan(Flo[:,0]))
                print ("GM.number()=" + str(np.sum(~np.isnan(Flo[:,0]))) + "  è lo stesso? " + str(A_float[ivar,itime,iSub,6]) )
              else:
...
#                print (A_float[3,:,6,6])
            ...
            # CALCULATE THE BIAS, RMSD AND CORR OF STATISTICS
                BIAS_matrix[ivar,itime,iSub,iStat] = np.nanmean(Mod[:,iStat]-Flo[:,iStat]) 
                RMSD_matrix[ivar,itime,iSub,iStat] = np.sqrt(np.nanmean((Mod[:,iStat]-Flo[:,iStat])**2)) 
                CORR_matrix[ivar,itime,iSub,iStat] = np.corrcoef(Mod[:,iStat],Flo[:,iStat])[0,1]

    print (var)
#    if (var=="P_l"):
#    A_float[ivar,0:3,iSub,2] = np.nan
 
np.save(OUTDIR + 'Basin_Statistics_FLOAT',A_float)
np.save(OUTDIR + 'Basin_Statistics_MODEL',A_model)
np.save(OUTDIR + 'Basin_Statistics_BIAS',BIAS_matrix)
np.save(OUTDIR + 'Basin_Statistics_RMSD',RMSD_matrix)
np.save(OUTDIR + 'Basin_Statistics_CORR',CORR_matrix)
