...
FLOAT_OUT_N3n = ['6901767','6901771','6901773','6903197']
VARLIST = ['P_l','N3n','O2o']
Adj = [True,True,False]
#VARLIST = ['N3n','O2o']
#Adj = [True,False]
...
M_all = Matchup_Manager(ALL_PROFILES,TL,BASEDIR)
MonthlyRequestors=M_all.TL.getMonthlist()
nTime = len(MonthlyRequestors)

N3n_PROFILES = []
for ip, p in enumerate(ALL_PROFILES):
    if ( p.name() not in FLOAT_OUT_N3n):
...
for ivar, var_mod in enumerate(VARLIST):
	...
	for iSub, Sub in enumerate(OGS.basin_list):
	    BASIN_PROFILES_float_ALL = bio_float.FloatSelector(var,Req.time_interval,Sub)
#	    A_float[ivar,itime,iSub,6] = len(BASIN_PROFILES_float)
#            A_model[ivar,itime,iSub,6] = len(BASIN_PROFILES_float)
            BASIN_PROFILES_float = []
            for p in BASIN_PROFILES_float_ALL:
                if ( var_mod == "P_l"):  BASIN_PROFILES_float = BASIN_PROFILES_float_ALL
                if ((var_mod == "N3n") and (p.name() not in FLOAT_OUT_N3n)): BASIN_PROFILES_float.append(p)  
		if (var_mod == "O2o"):  BASIN_PROFILES_float = BASIN_PROFILES_float_ALL
	
   	    A_float[ivar,itime,iSub,6] = len(BASIN_PROFILES_float)
            A_model[ivar,itime,iSub,6] = len(BASIN_PROFILES_float)
	    if len(BASIN_PROFILES_float) == 0: continue
	    print "len = " + np.str(len(BASIN_PROFILES_float))
...
#                A_model[ivar,itime,iSub,6] = GM.Lengths
		...
		Flo[ip,5] = gm200.Ref[0] # Surf Value
                ...
                A_float[ivar,itime,iSub,6] = gm200.number()
                A_model[ivar,itime,iSub,6] = gm200.number()
	        NPOINTS = len(REF_LAYER_MEAN)
#                A_float[ivar,itime,iSub,6] = NPOINTS
#		A_model[ivar,itime,iSub,6] = NPOINTS
#		print "NPOINTS = " + np.str(NPOINTS)
#		print "------------------"
