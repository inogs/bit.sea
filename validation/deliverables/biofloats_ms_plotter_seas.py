...
def single_plot(longvar, var, sub, layer, timeinterval, clim_req):
...
SeasObj=season.season()
SeasObj.setseasons(["1001","0401"],["winter","summer"])

iSeas=SeasObj.SEASON_LIST_NAME.index(args.season)
Seas_req = timerequestors.Clim_season(iSeas,SeasObj)
for ivar, var in enumerate(VARLIST):
    ...
    for isub, sub in enumerate(SUBLIST):
        for ilayer, layer in enumerate(LAYERLIST):
            ...
            print outfile
            fig,bias,rmse,ax,ax2  = single_plot(VARLONGNAMES[ivar],var,sub.name,layer.string(), ti_restrict, Seas_req)
