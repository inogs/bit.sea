...
# QUID REANALYSIS
# SECTION 4:
export MASKFILE=/g100_work/OGS_prodC/OPA/V10C-prod/etc/static-data/MED24_125/meshmask.nc
export ONLINE_REPO=gss/gss_work/DRES_OGS_BiGe/Observations/TIME_RAW_DATA/ONLINE_V10C/


######################### MATCHUP PUNTUALE ####################

OUTDIR=matchup_validation_run04/density_plots
...
python density_plots.py   -M $MASKFILE  -o $OUTDIR/Dens_N4n  -v N3n -m 0 -c supercoastal
...
OUTDIR=matchup_validation_run04
...
python vertical_profiles.py -m $MASKFILE -o $OUTDIR/Vert_Prof/N1p  -v N1p -c supercoastal | grep corr > $OUTDIR/table.N1p_corr.dat
python vertical_profiles.py -m $MASKFILE -o $OUTDIR/Vert_Prof/N3n  -v N3n -c supercoastal | grep corr > $OUTDIR/table.N3n_corr.dat
python vertical_profiles.py -m $MASKFILE -o $OUTDIR/Vert_Prof/N4n  -v N4n -c supercoastal | grep corr > $OUTDIR/table.N4n_corr.dat
python vertical_profiles.py -m $MASKFILE -o $OUTDIR/Vert_Prof/N5s  -v N5s -c supercoastal | grep corr > $OUTDIR/table.N5s_corr.dat
python vertical_profiles.py -m $MASKFILE -o $OUTDIR/Vert_Prof/O2o  -v O2o -c supercoastal | grep corr > $OUTDIR/table.O2o_corr.dat
python vertical_profiles.py -m $MASKFILE -o $OUTDIR/Vert_Prof/P_l  -v P_l -c supercoastal | grep corr > $OUTDIR/table.P_l_corr.dat
