...
F0v2wrong = timelistcontainer(TI_V2,ARCHIVE_DIR_WRONG,'f0')
F1v2wrong = timelistcontainer(TI_V2,ARCHIVE_DIR_WRONG,'f1')
F2v2wrong = timelistcontainer(TI_V2,ARCHIVE_DIR_WRONG,'f2')
...
for sub in [OGS.alb]:
    ...
    ax1.plot(times,f0v1,'b', label='V1 chain' )
    
    times, f0_w= F0v2wrong.plotdata(F0v2wrong.bias, sub.name,coast)
    ax1.plot(times,f0_w,'-r',label='Wr: $1^{st}$ day of forecast (misfit DA d+7)')
    # f1  v2 wrong 
    times, f1_w= F1v2wrong.plotdata(F1v2wrong.bias, sub.name,coast)
    ax1.plot(times,f1_w,'--r',label='Wr: $2^{nd}$ day of forecast')
    # f2  v2 wrong 
    times, f2_w= F2v2wrong.plotdata(F2v2wrong.bias, sub.name,coast)
    ...
    #RMS
    # f0  v2 wrong 
    times, f0_w= F0v2wrong.plotdata(F0v2wrong.rmse, sub.name,coast)
    ax2.plot(times,f0_w,'-r')
    # f1  v2 wrong 
    times, f1_w= F1v2wrong.plotdata(F1v2wrong.rmse, sub.name,coast)
    ax2.plot(times,f1_w,'--r') 
    # f2  v2 wrong 
    times, f2_w= F2v2wrong.plotdata(F2v2wrong.rmse, sub.name,coast)
    ...
    # n. of valid points
    # f0  v2 wrong 
    times, f0_w= F0v2wrong.plotdata(F0v2wrong.number, sub.name,coast)
    ax3.plot(times,f0_w,'-r')
    # f1  v2 wrong 
    times, f1_w= F1v2wrong.plotdata(F1v2wrong.number, sub.name,coast)
    ax3.plot(times,f1_w,'--r')
    # f2  v2 wrong 
    times, f2_w= F2v2wrong.plotdata(F2v2wrong.number, sub.name,coast)
